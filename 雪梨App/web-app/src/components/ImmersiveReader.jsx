...
export default function ImmersiveReader({ storyId, onBack }) {
    ...
    return parts.map((part, index) => {
        ...
        return (
          <div key={`${paragraphId}-dialogue-${index}`} className={`w-full flex ${isLeft ? 'justify-start' : 'justify-end'} my-2`}>
            <motion.div 
              ...
              className={`flex items-end gap-2.5 max-w-[70%] ${isLeft ? '' : 'flex-row-reverse'}`}
            >
              {isLeft && character && (
      ...
      return (
        <motion.div 
          ...
          className="text-base leading-7 text-gray-700 my-2 px-1"
        >
          {part.text}
        </motion.div>
      );
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-[#FAF9F6] to-[#F5F3EF] font-sans">
      ...
      {/* Chat Area */}
      <div className={`flex-1 overflow-y-auto px-4 py-4 space-y-2 ${isAuthorMode ? 'bg-gray-900' : 'bg-transparent'}`}>
        {messages.map((msg) => (
          <motion.div
            key={msg.id}
              ...
              {chapter.ending.type === 'finished' ? (
                ...
                <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-gray-100">
                  <div className="flex items-center gap-2 mb-3">
                    <Sparkles size={18} className="text-sherry-500" />
                    <h3 className="font-bold text-gray-800">剧情选择</h3>
                  </div>
                  <p className="text-sm text-gray-600 mb-4">{chapter.ending.text}</p>
                  <div className="space-y-2">
                    {chapter.ending.options.map((option) => (
                      <motion.button
                        ...
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        className="w-full text-left px-4 py-3 bg-gradient-to-r from-sherry-50 to-orange-50 hover:from-sherry-100 hover:to-orange-100 rounded-xl border border-sherry-200 transition-all duration-200 flex items-center justify-between group"
                      >
      ...
      <AnimatePresence>
        {showIntimacyChange && (
          ...
          >
            <div className="bg-gray-800/90 backdrop-blur-sm text-white px-4 py-3 rounded-xl shadow-lg flex items-center gap-3">
              <Heart size={18} className="text-sherry-400" />
              <div>
                <p className="text-sm font-medium">{showIntimacyChange.characterName} 亲密状态变化</p>
                <p className="text-xs text-gray-300">
                  {showIntimacyChange.oldStatus} → {showIntimacyChange.newStatus}
                </p>
                <p className="text-xs text-gray-400 mt-0.5">{showIntimacyChange.description}</p>
              </div>
            </div>
          </motion.div>
        )}
